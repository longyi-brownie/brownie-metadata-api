...
async def list_stats(
    ...
    # Get stats
    stats_list = (
        query.order_by(Stats.timestamp.desc()).limit(pagination.limit + 1).all()
    )
    ...
    return PaginatedResponse(
        items=stats_list, next_cursor=next_cursor, has_more=has_more
    )
...
async def delete_stats(
    ...
    logger.info("Stats deleted", stats_id=stats.id, deleted_by=current_user.id)
