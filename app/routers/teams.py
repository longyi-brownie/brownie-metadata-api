...
async def list_teams(
    ...
    teams = db.query(Team).filter(Team.org_id == org_id, Team.is_active).all()
...
async def update_team(
    ...
    logger.info("Team updated", team_id=team.id, updated_by=current_user.id)
...
async def add_team_member(
    ...
    return TeamMemberResponse(user_id=user.id, role=user.role, user=user)
...
async def update_team_member(
    ...
    if not user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="User not found in team"
        )
