...
class TestIntegration:
    ...
    def test_full_user_lifecycle(self, client: TestClient, db_session: Session):
        ...
        org_users_response = client.get(f"/api/v1/organizations/{org_id}/users", headers=headers)
        ...
        create_user_response = client.post(f"/api/v1/organizations/{org_id}/users", json={
        ...
        # 7. Update user
        update_response = client.put(f"/api/v1/users/{second_user_id}", json={
        ...
        # 8. Delete user
        delete_response = client.delete(f"/api/v1/users/{second_user_id}", headers=headers)
        ...
        deleted_user_response = client.get(f"/api/v1/users/{second_user_id}", headers=headers)
        assert deleted_user_response.status_code == 404

    def test_multi_organization_isolation(self, client: TestClient, db_session: Session):
        """Test that users from different organizations are properly isolated."""
        # Create two users in different organizations
        signup1 = client.post("/api/v1/auth/signup", json={
        ...
        signup2 = client.post("/api/v1/auth/signup", json={
        ...
        org2_users = client.get(f"/api/v1/organizations/{org2_id}/users", headers=headers1)
        ...
        org1_users = client.get(f"/api/v1/organizations/{org1_id}/users", headers=headers2)
    ...
    def test_pagination(self, client: TestClient, db_session: Session):
        ...
        for i in range(10):
            client.post(f"/api/v1/organizations/{org_id}/users", json={
        ...
        # Test pagination
        response = client.get(f"/api/v1/organizations/{org_id}/users?limit=5", headers=headers)
    ...
    def test_database_consistency(self, client: TestClient, db_session: Session):
        ...
        duplicate_response = client.post("/api/v1/auth/signup", json={
            "email": "duplicate@example.com",
            "password": "testpassword123",
            "username": "duplicate2",
        ...
        duplicate_username = client.post(f"/api/v1/organizations/{org_id}/users", json={
