...
def test_complete_user_crud():
    ...
    if me_response.status_code == 200:
        ...
        print(
            f"   ❌ Get current user failed: {me_response.status_code} - {me_response.text}"
        )
        return
    ...
    if user_response.status_code == 200:
        ...
        print(
            f"   ❌ Get user failed: {user_response.status_code} - {user_response.text}"
        )
    ...
    if org_users_response.status_code == 200:
        ...
        print(
            f"   ❌ List organization users failed: {org_users_response.status_code} - {org_users_response.text}"
        )
    ...
    if signup_response.status_code == 200:
        ...
        print(
            f"   ✅ Signup successful! New user created with token: {signup_data['access_token'][:50]}..."
        )
    else:
        print(
